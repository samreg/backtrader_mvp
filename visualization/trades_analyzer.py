...
'best_trade': round(best_trade, 2),
    ...
    def get_trade_details(self) -> pd.DataFrame:
        ...
        exits = df[df["event_type"].astype(str).str.startswith("EXIT")].copy()
        if exits.empty:
            entry_time["pnl"] = 0.0
        else:
    ...
    def compute_heatmaps(self) -> Dict[str, Any]:
        """
        Compute temporal heatmaps (hour of day, day of week)
        
        Returns:
            Dict with heatmap data
        """
        if self.trades is None or len(self.trades) == 0:
            return self._empty_heatmaps()
        
        trades = self.trades
        
        # Get ENTRY events only
        entries = trades[trades['event_type'] == 'ENTRY'].copy()
        ...
        entries['day_of_week'] = entries['datetime'].dt.dayofweek
        
        # Get exit PnL for each trade
        exit_events = trades[trades['event_type'].isin(['SL', 'BE', 'TP1', 'TP2'])]
        trade_pnl = exit_events.groupby('trade_id')['pnl'].sum()
        
        entries = entries.merge(
            trade_pnl.rename('total_pnl'),
            left_on='trade_id',
            right_index=True,
            how='left'
        )
        entries['total_pnl'] = entries['total_pnl'].fillna(0)
        
        # Hour of day heatmap
        hour_stats = entries.groupby('hour').agg({
            'trade_id': 'count',
            'total_pnl': 'sum'
        }).rename(columns={'trade_id': 'count', 'total_pnl': 'pnl'})
        ...
        for hour in range(24):
            if hour in hour_stats.index:
                hour_heatmap.append({
                    'hour': hour,
                    'count': int(hour_stats.loc[hour, 'count']),
                    'pnl': float(hour_stats.loc[hour, 'pnl'])
                })
            else:
                hour_heatmap.append({
                    'hour': hour,
        ...
        day_stats = entries.groupby('day_of_week').agg({
            'trade_id': 'count',
            'total_pnl': 'sum'
        }).rename(columns={'trade_id': 'count', 'total_pnl': 'pnl'})
        ...
        for day_idx in range(7):
            if day_idx in day_stats.index:
                day_heatmap.append({
                    'day': day_names[day_idx],
                    'count': int(day_stats.loc[day_idx, 'count']),
                    'pnl': float(day_stats.loc[day_idx, 'pnl'])
                })
            else:
                day_heatmap.append({
                    'day': day_names[day_idx],
